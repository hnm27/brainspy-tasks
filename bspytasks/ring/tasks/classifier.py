...
def ring_task(configs, dataloaders, custom_model, criterion, algorithm, waveform_transforms=None, logger=None, is_main=True, save_data=True):
    results = {}
    results['gap'] = str(configs['data']['gap'])
    print('==========================================================================================')
    print("GAP: " + str(results['gap']))

    results_dir, reproducibility_dir = init_dirs(str(results['gap']), configs['results_base_dir'], is_main=is_main, save_data=save_data)
...
def plot_inputs(results, label, colors=['b', 'r'], plots_dir=None, extension='png'):
    ...
    plt.scatter(inputs[targets == 0][:, 0], inputs[targets == 0][:, 1], marker='.', c=colors[0], label='Class 0 (' + label + ')', cmap=colors[0])
    plt.scatter(inputs[targets == 1][:, 0], inputs[targets == 1][:, 1], marker='x', c=colors[1], label='Class 1 (' + label + ')', cmap=colors[1])
