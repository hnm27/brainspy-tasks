...
class DefaultCustomSimulationModel(DNPU):
    def __init__(self, configs):
        ...
        super(DefaultCustomSimulationModel, self).__init__(
            Processor(configs, model_data['info'],
                      model_data['model_state_dict']),
            [configs['input_indices']])
    ...
    def forward(self, x):
        cv = self.get_control_voltages()
        cv[0, -1] = 0.
        self.set_control_voltages(cv)
        x = super().forward(x)
        return x
...
class DefaultCustomHardwareModel(DNPU):
    def __init__(self, configs):
        ...
        super(DefaultCustomHardwareModel,
              self).__init__(Processor(configs),
