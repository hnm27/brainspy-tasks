...
class Hardware_Validator:
    def __init__(self, configs):
        ...
        assert (
            self.slope_length > 0
        ), f"Slopes cannot be zero! slope_length=={self.slope_length}"
        assert (
            self.plateau_lengths > 10
        ), f"Input plateaus cannot be zero! plateau_lengths=={self.plateau_lengths}"
    ...
    def generate_sloped_values(self, values, total_length):
        return (
            values[np.newaxis]
            * generate_slopped_plato(self.slope_length, total_length)[:, np.newaxis]
        )
    ...
    def validate_prediction(self, name, inputs, control_voltages, predictions, mask):
        ...
        inputs = self.clip(
            inputs, max_value=MAX_CLIPPING_VALUE, min_value=MIN_CLIPPING_VALUE
        )
        control_voltages = self.clip(
            control_voltages, max_value=MAX_CLIPPING_VALUE, min_value=MIN_CLIPPING_VALUE
        )
        ...
        error = (
            (measurement[mask, 0] - predictions[mask]) ** 2
        ).mean()  # TODO: predictions should be 2 dim array with last dim singleton
