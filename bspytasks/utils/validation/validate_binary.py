...
def validate_outputs(configs):
    ...
    for n, cv in enumerate(control_voltages_array):
        ...
        if len(np.unique(gate_array[n])) == 1:
            ...
            mserror, _, measurement = validator.validate_prediction(
                name, inputs, cv, predictions[n], mask
            )
            plt_name = os.path.join(validator.validation_dir, name)
            accuracy, predicted_labels, threshold = perceptron(
                measurement, targets_array[n], plot=plt_name
            )
            correlation_array[n] = corr_coeff(measurement.T, targets_array[n].T)
            mserror_array[n] = mserror
            print(
                f"{name} has accuracy {accuracy:.2f} % and correlation {correlation_array[n]:.2f}"
            )
...
def validate_capacity(configs):
    ...
    for vc in configs["from_to_vcdim"]:
        ...
        capacity, correlation_array, accuracy_array, mserror_array = validate_outputs(
            configs_vc
        )
        ...
        validation_summary["validation_prediction_corr_per_N"].append(
    ...
    dimensions = np.arange(
        configs["from_to_vcdim"][0], configs["from_to_vcdim"][-1] + 1
    )
    ...
    plot_boxplot(
        validation_summary["accuracy_distib_per_N"],
        dimensions,
        configs["data_dir"],
        title="Validated accuracy",
    )
    plot_boxplot(
        validation_summary["mserror_distrib_per_N"],
        dimensions,
        configs["data_dir"],
        title="MSE over N points",
    )
    plot_boxplot(
        validation_summary["validation_prediction_corr_per_N"],
        dimensions,
        configs["data_dir"],
        title="Correlation prediction and measurement",
    )
